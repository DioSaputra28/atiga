                        ...
                        @foreach($regulations as $index => $regulation)
                            <div class="accordion-item overflow-hidden rounded-lg border border-slate-200">
                                ...
                                <div id="accordion-{{ $index }}" class="accordion-content hidden bg-white px-4 py-4 text-sm text-slate-600">
                                    ...
                                    @if(filled($regulation['document_url']))
                                        <a href="{{ $regulation['document_url'] }}" target="_blank" rel="noopener noreferrer" class="mt-3 inline-flex items-center gap-2 rounded-md bg-primary-700 px-3 py-2 text-xs font-semibold text-white transition hover:bg-primary-600">
                                            <i class="fa-solid fa-file-arrow-down"></i>
                                            {{ $regulation['document_name'] }}
                                        </a>
                                    @endif

                                    @if(filled($regulation['youtube_embed_url']))
                                        <div class="mt-4 overflow-hidden rounded-lg border border-slate-200">
                                            <iframe
                                                src="{{ $regulation['youtube_embed_url'] }}"
                                                title="Video penjelasan {{ $regulation['title'] }}"
