...
<div class="w-full bg-white border-b border-slate-200">
    <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-3 py-2 sm:px-4 sm:py-3">
        <div class="flex items-center gap-3">
            @if ($companyLogoUrl)
                <img src="{{ $companyLogoUrl }}" alt="{{ $companyName }}" class="h-9 w-9 rounded-lg object-cover sm:h-11 sm:w-11">
            @else
                <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-primary-700 text-white sm:h-11 sm:w-11">
                    <i class="fa-solid fa-scale-balanced text-lg"></i>
                </div>
            @endif
            <div>
                <p class="text-lg font-extrabold tracking-tight text-primary-700 sm:text-xl">{{ $companyName }}</p>
...
<nav class="sticky top-0 z-20 bg-primary-700 shadow-md" id="main-nav">
    <div class="mx-auto flex max-w-7xl items-center justify-between px-3 py-2 text-sm font-semibold text-white sm:px-4 sm:py-3">
        {{-- Desktop Navigation --}}
        ...
        <button type="button" id="mobile-menu-toggle"
                class="inline-flex min-h-[44px] min-w-[44px] items-center justify-center rounded-md p-2 text-white hover:text-secondary-500 focus:outline-none md:hidden"
    ...
    <div class="hidden md:hidden" id="mobile-menu">
        <div class="border-t border-primary-600 bg-primary-700 px-4 py-3">
            <div class="flex flex-col space-y-3">
                ...
                <a href="{{ $articlesRoute }}"
                   class="rounded-md px-3 py-3 text-sm font-medium transition {{ request()->routeIs('articles.*') ? 'bg-primary-600 text-secondary-500' : 'text-white hover:bg-primary-600 hover:text-secondary-500' }}">
                    <i class="fa-solid fa-newspaper mr-2 w-5"></i> Artikel
                </a>
                <a href="{{ $trainingsRoute }}"
                   class="rounded-md px-3 py-3 text-sm font-medium transition {{ request()->routeIs('trainings.*') ? 'bg-primary-600 text-secondary-500' : 'text-white hover:bg-primary-600 hover:text-secondary-500' }}">
                    <i class="fa-solid fa-graduation-cap mr-2 w-5"></i> Training
                </a>
                <a href="{{ $activitiesRoute }}"
                   class="rounded-md px-3 py-3 text-sm font-medium transition {{ request()->routeIs('activities.*') ? 'bg-primary-600 text-secondary-500' : 'text-white hover:bg-primary-600 hover:text-secondary-500' }}">
                    <i class="fa-solid fa-calendar-days mr-2 w-5"></i> Aktifitas
                </a>
                <a href="{{ $contactRoute }}"
                   class="mt-2 rounded-md border border-secondary-500 px-3 py-3 text-center text-xs font-semibold text-secondary-500 transition hover:bg-secondary-500 hover:text-primary-700">
                    Konsultasi Gratis
                </a>
            </div>
        </div>
